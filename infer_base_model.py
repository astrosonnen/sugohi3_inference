...
# defines prior on lmchab (fixed).
def lmchab_prior_func(m): # skew Gaussian distribution in stellar mass
    return 1./(2.*np.pi)**0.5/mchab_sig*np.exp(-0.5*(m - mchab_mu)**2/mchab_sig**2) * (1. + erf(10.**mchab_logskew*(m - mchab_mu)/2.**0.5/mchab_sig))

nmchab_grid = 1001
lmchab_grid = np.linspace(10., 13., nmchab_grid)

lmchab_spline = splrep(lmchab_grid, lmchab_prior_func(lmchab_grid))
lmchab_intfunc = lambda x: splint(lmchab_grid[0], x, lmchab_spline)
lmchab_norm = lmchab_intfunc(lmchab_grid[-1])
...
# reads in the MCMC of a weak lensing inference of CMASS, to use as a prior on m200.
...
pars = [m200_mu, m200_beta, m200_sig, zs_mu, zs_sig, aimf_mu, aimf_sig]
...
def norm_func(p):
    ...
    m200_mu, m200_beta, m200_sig, zs_mu, zs_sig, aimf_mu, aimf_sig = p
    ...
    integrand = cs_m200_term*cs_aimf_term*cs_zs_term*cs_samp['crosssect']/cs_m200_ip/cs_aimf_ip
...
def logpfunc(p):
    ...
    for i in range(nsl):
        ...
        sumlogp += np.log((lmchab_like_term*sl_like_term*zs_term*crosssect_here/norm).mean())
...
output_file = h5py.File('base_inference.hdf5', 'w')
