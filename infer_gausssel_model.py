...
tsel_mu = {'name': 'tsel_mu', 'lower': 0., 'upper': 3., 'guess': 1.2, 'step': 0.1}
tsel_sig = {'name': 'tsel_sig', 'lower': 0., 'upper': 1., 'guess': 0.4, 'step': 0.1}

pars = [m200_mu, m200_beta, m200_sig, zs_mu, zs_sig, aimf_mu, aimf_sig, tsel_mu, tsel_sig]
...
def tsel_func(tein, tsel_mu, tsel_sig):
    return 1./(2.*np.pi)**0.5 / tsel_sig * np.exp(-0.5*(tein - tsel_mu)**2/tsel_sig**2)

norm_min = 0.01

def norm_func(p):

    m200_mu, m200_beta, m200_sig, zs_mu, zs_sig, aimf_mu, aimf_sig, tsel_mu, tsel_sig = p
    ...
    integrand = cs_m200_term*cs_aimf_term*cs_zs_term*cs_samp['crosssect']*tsel_func(cs_samp['tein'], tsel_mu, tsel_sig)/cs_m200_ip/cs_aimf_ip
...
def logpfunc(p):
    ...
    for i in range(nsl):
        ...
        sumlogp += np.log((lmchab_like_term*sl_like_term*zs_term*crosssect_here*tsel_func(tein_here, tsel_mu, tsel_sig)/norm).mean())
...
output_file = h5py.File('gausssel_inference.hdf5', 'w')
